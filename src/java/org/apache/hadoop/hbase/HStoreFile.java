...
public class HStoreFile implements HConstants {
    ...
    HStoreFile(HBaseConfiguration conf, FileSystem fs, Path basedir,
        ...
        // zeng: encoded region name
        this.encodedRegionName = encodedRegionName;
        // zeng: column family name
    ...
    long loadInfo(FileSystem fs) throws IOException {
        Path p = null;
        if (isReference()) {
            p = getInfoFilePath(reference.getEncodedRegionName(), reference.getFileId(), null);
        } else {
            p = getInfoFilePath();
        }
    ...
    public synchronized MapFile.Reader getReader(final FileSystem fs,
        ...
        if (isReference()) {
            return new HStoreFile.HalfMapFileReader(
                    fs, getMapFilePath(reference).toString(), conf, reference.getFileRegion(), reference.getMidkey(), bloomFilter
            );
        }
    ...
    private static String createHStoreFilename(final long fid, final String encodedRegionName) {
        // zeng: file id . encoded region name
        return Long.toString(fid) + ((encodedRegionName != null) ? "." + encodedRegionName : "");
    }
    ...
    static class Reference implements Writable {
        ...
        public void readFields(DataInput in) throws IOException {
            // zeng: 引用的region的encoded region name
